...
import asyncio
import websockets
from core.rpp_engine import RPPEngine
...
class QuantumEngine:
    ...
    def __init__(self, host='localhost', port=8765):
        # Initialize RPP Engine
        self.rpp_engine = RPPEngine(local_template_path='templates', github_repo_url='https://github.com/ppgmuhammadsalam96930-design/STL-UDHIS-AI')
        self.host = host
        self.port = port
    ...
    def _setup_routes(self):
        # RPP routes
        self.message_router.register_route('rpp_list_templates', self._handle_rpp_list_templates)
        self.message_router.register_route('rpp_generate', self._handle_rpp_generate)
        """Setup message routing"""
        self.message_router.register_route('ping', self._handle_ping)

        # Extra app-specific routes
        self.message_router.register_route('translate', self._handle_translate)
        self.message_router.register_route('detect_spider', self._handle_detect_spider)
        self.message_router.register_route('save_grade', self._handle_save_grade)
        self.message_router.register_route('save_finance', self._handle_save_finance)
        self.message_router.register_route('save_schedule', self._handle_save_schedule)
        self.message_router.register_route('export_data', self._handle_export_data)
...
if __name__ == "__main__":
    ...
    async def _handle_rpp_list_templates(self, message, websocket):
        local = self.rpp_engine.list_local_templates()
        github = self.rpp_engine.list_github_templates(templates_path="templates")
        return {"type":"rpp_list_templates_response", "local": local, "github": github}

    async def _handle_rpp_generate(self, message, websocket):
        t = message.get("template")
        ctx = message.get("context", {})
        opts = message.get("options", {})
        res = self.rpp_engine.generate(
            template_name=t,
            ctx=ctx,
            prefer_local=opts.get("prefer_local", True),
            save_markdown=opts.get("save_markdown"),
            save_pdf=opts.get("save_pdf")
        )
        return {"type":"rpp_generate_response", "result": res}


    # -------------------------
    # New handlers: translate, detect_spider, save_grade/finance/schedule, export_data
    # -------------------------
    async def _handle_translate(self, message, websocket):
        """
        message:
        { "type":"translate", "text":"...", "from": "id", "to":"en" }
        """
        text = message.get("text", "")
        src = message.get("from", None)
        tgt = message.get("to", None)
        # If a translator provider is configured via env, attempt to use it
        api = os.environ.get("TRANSLATOR_API_URL")
        api_key = os.environ.get("TRANSLATOR_API_KEY")
        if api and text:
            try:
                payload = {"text": text, "from": src, "to": tgt, "key": api_key}
                import requests
                r = requests.post(api, json=payload, timeout=10)
                if r.status_code == 200:
                    data = r.json()
                    return {"type":"translate_response", "ok":True, "result": data}
                else:
                    return {"type":"translate_response", "ok":False, "error":"remote_translator_failed", "status": r.status_code}
            except Exception as e:
                return {"type":"translate_response", "ok":False, "error": str(e)}
        # fallback: simple pseudo-translation (identity or reversed)
        if not text:
            return {"type":"translate_response", "ok":False, "error":"no_text"}
        # simple heuristic: if target is different just return original with note
        note = f"[translator_unavailable_fallback]"
        return {"type":"translate_response", "ok":True, "result": {"translated_text": text, "note": note}}

    async def _handle_detect_spider(self, message, websocket):
        """
        message: { "type":"detect_spider", "metrics": {"m1":val, "m2":val, ...}, "title":"..." }
        Generates a radar/spider chart image and returns its path (and base64)
        """
        metrics = message.get("metrics", {})
        title = message.get("title", "spider")
        if not metrics:
            return {"type":"detect_spider_response", "ok":False, "error":"no_metrics"}
        # prepare outputs folder
        outdir = os.path.join(os.getcwd(), "outputs")
        os.makedirs(outdir, exist_ok=True)
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        img_name = f"spider_{title}_{timestamp}.png"
        img_path = os.path.join(outdir, img_name)
        if MATPLOTLIB_AVAILABLE:
            try:
                labels = list(metrics.keys())
                values = [float(metrics[k]) for k in labels]
                # Radar chart: repeat first value to close the loop
                angles = list(range(len(values)))
                import math
                ...
                return {"type":"detect_spider_response", "ok":True, "path": img_path, "b64": b64}
            except Exception as e:
                return {"type":"detect_spider_response", "ok":False, "error": str(e)}
        else:
            return {"type":"detect_spider_response", "ok":False, "error":"matplotlib_not_available"}

    async def _handle_save_grade(self, message, websocket):
        """
        message: { "type":"save_grade", "student_id": "...", "grade": {...} }
        """
        payload = message.get("grade", {})
        sid = message.get("student_id", "unknown")
        outdir = os.path.join(os.getcwd(), "outputs", "grades")
        os.makedirs(outdir, exist_ok=True)
        fname = os.path.join(outdir, f"{sid}.json")
        try:
            with open(fname, "w", encoding="utf-8") as f:
                json.dump({"student_id": sid, "grade": payload, "ts": datetime.datetime.now().isoformat()}, f, ensure_ascii=False, indent=2)
            return {"type":"save_grade_response", "ok":True, "path": fname}
        except Exception as e:
            return {"type":"save_grade_response", "ok":False, "error": str(e)}

    async def _handle_save_finance(self, message, websocket):
        """
        message: { "type":"save_finance", "account":"...", "data": {...} }
        """
        data = message.get("data", {})
        account = message.get("account", "default")
        outdir = os.path.join(os.getcwd(), "outputs", "finance")
        os.makedirs(outdir, exist_ok=True)
        fname = os.path.join(outdir, f"{account}.json")
        try:
            with open(fname, "w", encoding='utf-8') as f:
                json.dump({"account": account, "data": data, "ts": datetime.datetime.now().isoformat()}, f, ensure_ascii=False, indent=2)
            return {"type":"save_finance_response", "ok":True, "path": fname}
        except Exception as e:
            return {"type":"save_finance_response", "ok":False, "error": str(e)}

    async def _handle_save_schedule(self, message, websocket):
        """
        message: { "type":"save_schedule", "owner":"...", "schedule": {...} }
        """
        data = message.get("schedule", {})
        owner = message.get("owner", "default")
        outdir = os.path.join(os.getcwd(), "outputs", "schedules")
        os.makedirs(outdir, exist_ok=True)
        fname = os.path.join(outdir, f"{owner}.json")
        try:
            with open(fname, "w", encoding='utf-8') as f:
                json.dump({"owner": owner, "schedule": data, "ts": datetime.datetime.now().isoformat()}, f, ensure_ascii=False, indent=2)
            return {"type":"save_schedule_response", "ok":True, "path": fname}
        except Exception as e:
            return {"type":"save_schedule_response", "ok":False, "error": str(e)}

    async def _handle_export_data(self, message, websocket):
        """
        message: { "type":"export_data", "scope": "all"|"grades"|"finance"|"schedules", "format":"json"|"csv" }
        """
        scope = message.get("scope", "all")
        fmt = message.get("format", "json")
        base_out = os.path.join(os.getcwd(), "outputs", "exports")
        os.makedirs(base_out, exist_ok=True)
        timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
        outpath = os.path.join(base_out, f"export_{scope}_{timestamp}.{fmt}")
        try:
            collected = {}
            root = os.path.join(os.getcwd(), "outputs")
            if scope in ("all","grades"):
                grades_dir = os.path.join(root, "grades")
                if os.path.isdir(grades_dir):
                    for f in os.listdir(grades_dir):
                        with open(os.path.join(grades_dir, f), "r", encoding="utf-8") as fh:
                            collected.setdefault("grades", {})[f] = json.load(fh)
            if scope in ("all","finance"):
                fin_dir = os.path.join(root, "finance")
                if os.path.isdir(fin_dir):
                    for f in os.listdir(fin_dir):
                        with open(os.path.join(fin_dir, f), "r", encoding="utf-8") as fh:
                            collected.setdefault("finance", {})[f] = json.load(fh)
            if scope in ("all","schedules"):
                sch_dir = os.path.join(root, "schedules")
                if os.path.isdir(sch_dir):
                    for f in os.listdir(sch_dir):
                        with open(os.path.join(sch_dir, f), "r", encoding="utf-8") as fh:
                            collected.setdefault("schedules", {})[f] = json.load(fh)
            if fmt == "json":
                with open(outpath, "w", encoding="utf-8") as f:
                    json.dump(collected, f, ensure_ascii=False, indent=2)
                return {"type":"export_data_response", "ok":True, "path": outpath}
            elif fmt == "csv":
                # simple CSV export if pandas available
                if PANDAS_AVAILABLE:
                    rows = []
                    # flatten for grades as example
                    for key, val in collected.get("grades", {}).items():
                        sid = key
                        data = val.get("grade", {})
                        flat = {"student_id": sid}
                        flat.update(data if isinstance(data, dict) else {"value": data})
                        rows.append(flat)
                    df = pd.DataFrame(rows)
                    df.to_csv(outpath, index=False)
                    return {"type":"export_data_response", "ok":True, "path": outpath}
                else:
                    return {"type":"export_data_response", "ok":False, "error":"pandas_not_available"}
            else:
                return {"type":"export_data_response", "ok":False, "error":"unsupported_format"}
        except Exception as e:
            return {"type":"export_data_response", "ok":False, "error": str(e)}
